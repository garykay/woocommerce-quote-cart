...
echo sprintf( __( 'You have received an quote from %s. Their quote is as follows:', 'woocommerce' ), $order->billing_first_name . ' ' . $order->billing_last_name ) . "\n\n";

echo "****************************************************\n\n";

do_action( 'woocommerce_email_before_order_table', $order, true );

echo sprintf( __( 'quote number: %s', 'woocommerce'), $order->get_order_number() ) . "\n";
echo sprintf( __( 'quote date: %s', 'woocommerce'), date_i18n( __( 'jS F Y', 'woocommerce' ), strtotime( $order->order_date ) ) ) . "\n";
